...
const calculateDocumentStats = (docs) => {
  ...
  // FIXED: Calculate progress from RM perspective
  // A document is "completed" from RM perspective when:
  // 1. RM has marked it as "submitted_for_review" (or similar submitted status)
  // 2. OR it's in a state where RM doesn't need to act (like sighted, waived, tbo, etc.)
  // 3. BUT documents marked as "pending_from_customer" or similar pending status should NOT count as complete
...
const RmReviewChecklistModal = ({ checklist, open, onClose, refetch }) => {
  ...
  //               }
  //               description={
  //                 <div className="wrap-break-word">{item.message}</div>
  //               }
  //             />
  //           </List.Item>
  //         )}
  ...
  // CommentTrail duplicate removed and replaced by shared CommentHistory component
  ...
  const downloadChecklistAsPDF = async () => {
      ...
      <div class="pdf-container">
        ...
        ${
          comments && comments.length > 0
            ? `
          <div class="section-card">
            <div class="section-title">Comment History (Last ${Math.min(comments.length, 3)})</div>
            ${comments
              .slice(0, 3)
              .map(
                (comment) => `
              <div class="comment-box" style="margin-bottom: 6px;">
                <div class="comment-header">
                  <span class="comment-author">${
                    comment.userId?.name || "System User"
                  }</span>
                  <span class="comment-date">
                    ${dayjs(comment.createdAt || comment.timestamp).format("DD MMM, HH:mm")}
                  </span>
                </div>
    ...
    `;
      ...
      const imgWidth = 280; // Slightly less than full A4 landscape width
      const imgHeight = (canvas.height * imgWidth) / canvas.width;

      // Single page PDF - no pagination needed
          ...
          <>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                {/* Show uploaded supporting documents next to the button */}
