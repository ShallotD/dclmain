...
// // Helper: Send reminder email to approver (calls backend)
// const sendReminderEmail = async (deferralId, approverName) => {
//   try {
//     message.loading(`Sending reminder to ${approverName || "approver"}...`, 0);
//     const res = await deferralApi.sendReminder(deferralId);
//     if (res && res.success) {
//       message.success(
//         `Reminder sent to ${approverName || res.email || "approver"}`
//       );
//       return { success: true, recipient: res.email || res.recipient };
//     }
...
//     const current =
//       localDeferral.currentApprover ||
//       (localDeferral.approverFlow && localDeferral.approverFlow[0]);
//     const email =
//       current?.email ||
//       (typeof current === "string" && current.includes("@") ? current : "");
//     const name =
//       current?.name || (typeof current === "string" ? current : "Approver");
//     if (!email) {
//       message.warning("No email for current approver");
//       return;
//     }
//     setSendingReminder(true);
//     const res = await sendReminderEmail(localDeferral._id, name);
...
//         notes: `Reminder sent to ${name}`,
//         comment: `Reminder sent to ${name}`,
//       };
...
//                 {status === "deferral_requested" ? "Pending" : status}
//               </div>
//             </Descriptions.Item>
...
//                               ? `• ${dayjs(uploadedVersion.uploadDate).format(
//                                   "DD MMM YYYY HH:mm"
//                                 )}`
//                               : ""}
//                           </div>
...
//                   ✓ DCL document ready: <b>{dclDocs[0].name}</b>
//                   {dclDocs.length > 1 ? ` (+${dclDocs.length - 1} more)` : ""}
//                 </Text>
//               </div>
//             </>
//           ) : (
...
//             >
//               <UploadOutlined
//                 style={{ fontSize: 18, marginBottom: 6, color: WARNING_ORANGE }}
//               />
...
//               >
//                 <Text type="success" style={{ fontSize: 12 }}>
//                   ✓ {uploadedDocs.length} additional document
//                   {uploadedDocs.length !== 1 ? "s" : ""} ready
//                 </Text>
//               </div>
//             </>
//           ) : (
...
//                             <div>Current Approver • Pending Approval</div>
//                             {localDeferral.slaExpiry && (
//                               <span
...
//                                         user: "RM",
//                                         date: new Date().toISOString(),
//                                         notes: `Reminder email sent to ${
//                                           approver.split("@")[0]
//                                         } (${approver})`,
//                                         comment: `Reminder sent to ${
//                                           approver.split("@")[0]
//                                         }`,
//                                       };
//                                       if (onAction)
...
//                 >
//                   <BellOutlined
//                     style={{ color: WARNING_ORANGE, marginTop: 2 }}
//                   />
//                   <div>
//                     <Text
//                       strong
//                       style={{ fontSize: 13, color: WARNING_ORANGE }}
//                     >
//                       Send Reminder
//                     </Text>
//                     <Text
...
//                       }}
//                     >
//                       You can send a reminder email to the current approver if
//                       this deferral is pending approval for too long. The email
//                       will include deferral details and a direct link to the
//                       approval page.
//                     </Text>
//                   </div>
//                 </div>
...
//                 events.push({
//                   user: h.user?.name || h.user || h.user || "System",
...
//                 (d) => d.assignedRM && String(d.assignedRM._id) === String(rmId)
//               );
//             }
//           }
//         } catch (e) {
...
//       filteredData.filter(
//         (d) =>
//           (d.status || "").toLowerCase() !== "deferral_approved" &&
//           (d.status || "").toLowerCase() !== "deferral_rejected" &&
...
//   // Rejected should include both 'rejected' and 'deferral_rejected'
...
//         // If the updated deferral is rejected and belongs to this RM, switch to Rejected tab
...
//           />
//           <Tabs.TabPane
//             tab={`Rejected Deferrals (${rejectedData.length})`}
//             key="rejected"
//           />
//         </Tabs>
//       </div>
      ...
      >
        ...
        >
                ...
                return localDeferral.approvers
                  ...
                  .map((approver, index) => {
                    const isCurrentApprover = (() => {
                      if (
                        typeof localDeferral?.currentApproverIndex === "number"
                      )
                      ...
                      const getKey = (item) => {
                        ...
                        return (
                          ...
                          item.email ||
                          item.name ||
                          (item.user && (item.user.email || item.user.name)) ||
                          ""
                        ).toLowerCase();
                      };
          ...
          {(function () {
            ...
            if (
              !allApproversApprovedLocal &&
              (localDeferral.approverFlow || localDeferral.approvers)?.length >
                0
            ) {
              return (
                <div
